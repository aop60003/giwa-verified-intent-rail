...
| Flashblocks timeout or absence | Status rail fast feedback step | Standard RPC receipt status and block data remain the block-evidence source | Not counted as final confirmation | `confirmation.flashblocksExcludedFromFinalConfirmation=true` |
...
| Sprint 45 local hardening | `docs/evidence/bounded-failure-redaction-sprint45.json` | Raw verifier failures, hosted origin/content-type failures, public artifact markers, telemetry values, retained snapshot replay boundary, and copy are bounded | External blockers remain unchanged | `authority=local-advisory`, `canUnblockStaging=false` |
| Sprint 46 public boundary hardening | `docs/evidence/public-boundary-final-hardening-sprint46.json` | Legacy verifier failures are redacted on read paths, readiness metadata uses category labels, public evidence JSON is scanned, and public models use standard RPC block evidence wording | External blockers remain unchanged | `authority=local-advisory`, `canUnblockStaging=false` |
...
- Sprint 45 adds local hardening after Sprint 43 without changing the external blocker set or staging authorization.
- Sprint 46 adds final local public boundary hardening after Sprint 45 without changing the external blocker set or staging authorization.
