...
| Hosted ops release gate | `docs/implementation/giwa-hosted-ops-runbook.md` | Release checks exclude wallet action and chain-operation commands | Hosted beta remains blocked until gate passes | no public hosting from prototype mode |
...
| Sprint 19 source provenance gate | `docs/implementation/giwa-staging-release-provenance.md` | Non-git/no-CI mode blocks release promotion | Local checks are advisory until protected CI exists | `.git=False`, `.github=False` |
